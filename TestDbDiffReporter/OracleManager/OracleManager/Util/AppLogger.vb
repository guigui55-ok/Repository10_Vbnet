...
Public Class AppLogger
    ...
    Public Sub RecieveLogoutEvent(sender As Object, e As EventArgs)
    ...
    Private Sub Print(value As String)
        ...
        If value.Contains(vbCrLf) Then
            values = value.Split(vbCrLf)
        Else
            values = {value}
        End If
        For Each newValue As String In values
            newValue = newValue.Replace(vbCrLf, "")
            newValue = newValue.Replace(vbLf, "")
            _Print(newValue)
        Next
    End Sub

    Private Sub _Print(value As String)
        ...
        End If
        LastLog = value
        RaiseEvent AddLogEvent(value, EventArgs.Empty)
    End Sub
