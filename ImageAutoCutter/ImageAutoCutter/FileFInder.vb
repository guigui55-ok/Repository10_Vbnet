...
Public Class FileFInder
    ...
    Sub FindDirPaths()
        ...
        For Each pattern In _patterns
            For Each dirPath In _dirPaths
                FindPathToListDir(dirPath, pattern)
            Next
        Next
    ...
    Sub FindFilePaths()
        ...
        For Each pattern In _patterns
            For Each dirPath In _dirPaths
                FindPathToListFile(dirPath, pattern)
            Next
        Next
        _logger.Info($"_filePathList.Count = {_filePathList.Count}")
    End Sub

    Sub FindPathToListDir(dirPath As String, pattern As String)
        Dim files = Directory.GetFiles(dirPath, pattern, searchOption:=SearchOption.AllDirectories)
        _filePathList.AddRange(files)
    End Sub
    Sub FindPathToListFile(dirPath As String, pattern As String)
        Dim files = Directory.GetFiles(dirPath, pattern, searchOption:=SearchOption.AllDirectories)
        _filePathList.AddRange(files)
    End Sub

    Sub RemovePathRegex(regexPatterns() As String)
        Dim _list = New List(Of String)
        For Each fpath In _filePathList
            If Not RegexAnyUtil.IsMatchMultiple(fpath, regexPatterns) Then
                _list.Add(fpath)
            End If
        Next
        _filePathList = _list
